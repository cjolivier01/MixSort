...
class STrack(BaseTrack):
    ...
    def multi_predict(stracks):
        if len(stracks) > 0:
            multi_mean = torch.stack([st.mean.clone() for st in stracks])
            multi_covariance = torch.stack([st.covariance for st in stracks])
...
class HMTracker(object):
    ...
    def crop_and_resize(
        ...
        # x:left, y:top
        x = torch.round(x + 0.5 * w - crop_sz * 0.5)
        y = torch.round(y + 0.5 * h - crop_sz * 0.5)
