...
def run(args):
    ...
    seen_categories = set()
    data_path = os.path.dirname(args.input_file)
    OUT_PATH = data_path
    out_path = os.path.join(OUT_PATH, '{}.json'.format(os.path.basename(args.input_file)))
    ...
    out['videos'].append({'id': video_cnt, 'file_name': out_path})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Convert MOT to Coco")
    parser.add_argument( "--input-file", "-f", type=str, default=None, help="Input MOT file")
