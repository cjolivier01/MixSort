...
class HockeyTrackingDataset(BaseVideoDataset):
    """ HockeyTrackingDataset dataset.
    """
    def __init__(self, root=None, annotations=None, image_loader=jpeg4py_loader, split='train', data_fraction=None):
        """
        args:
            root        - The path to the HockeyTrackingDataset folder, containing the train, val, test sets.
        ...
        """
        self.root = env_settings().sportsmot_dir if root is None else root
        self.annotations=os.path.join(root,'annotations') if annotations is None else annotations
        super().__init__('hockeyTrackingDataset', root, image_loader)
    ...
    def _get_frame(self, seq_id, frame_id):
        if self.split == 'mix':
            s='train' if self.sequence_list[seq_id][0]=='0' else 'val'
            seq_name=self.sequence_list[seq_id][1:-4]
            frame_path = os.path.join(self.root,s,seq_name,'img1','{:0>6d}.jpg'.format(frame_id))
        else:
            seq_name=self.sequence_list[seq_id][:-4]
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        object_meta = OrderedDict({'object_class_name': 'player',
                                   'seq':self.sequence_list[seq_id][1:-4] if self.split == 'mix' else self.sequence_list[seq_id][:-4],
                                   'id':self.sequence_list[seq_id][-3:],
                                   'frames':frame_ids,})
